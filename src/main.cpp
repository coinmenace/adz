...
// Copyright (c) 2017-2018 The Adzcoin Core developers
...
#if defined(NDEBUG)
# error "Adzcoin Core cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "AdzcoinCoin Signed Message:\n";
...
CAmount GetBlockSubsidy(int nPrevBits, int nPrevHeight, const Consensus::Params& consensusParams, bool fSuperblockPartOnly)
{
    ...
    int nHeight = nPrevHeight + 1;
     CAmount nSubsidy = GetBlockReward(nHeight);
    //
    //return (nPrevHeight > 0) ? (nSubsidy - (nSubsidy * 0.1)) : 0;
    return (nHeight > 0) ? nSubsidy : 0;
}
...
CAmount GetBlockReward(int nPrevHeight){
    CAmount nSubsidy = 40 * COIN;

    if (nPrevHeight == 1)
        return 42000000 * COIN;

    nSubsidy >>= (nPrevHeight / 525600);
...
/*
 * 10% of block reward is reserved as founders reward
 *
 *
 */
CAmount GetFoundersReward(int nHeight)
{
    ...
    CAmount blockValue = GetBlockReward(nHeight);
...
/**
void addFoundersReward(const CChainParams& chainparams, const CScript& scriptPubKeyIn){
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck)
{
    ...
    // ADZCOIN : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    std::string strError = "";
    LogPrintf("ADZCOIN block.vtx[0].GetValueOut() %lld <= blockReward %lld\n", block.vtx[0].GetValueOut(), blockReward);
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(ADZCOIN): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(ADZCOIN): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // ADZCOIN : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(ADZCOIN): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(ADZCOIN): spork is off, skipping transaction locking checks\n");
    }
...
bool static AlreadyHave(const CInv& inv) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    /*
        Adzcoin Related Inventory Messages
