...
#include "config/adzcoin-config.h"
#endif
...
const char * const BITCOIN_CONF_FILENAME = "adzcoin.conf";
const char * const BITCOIN_PID_FILENAME = "adzcoind.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "adzcoin" is a composite category enabling all Adzcoin-related debug output
            if(ptrCategory->count(string("adzcoin"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AdzcoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AdzcoinCore
    // Mac: ~/Library/Application Support/AdzcoinCore
    // Unix: ~/.adzcoincore
#ifdef WIN32
    // Windows
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty adzcoin.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override adzcoin.conf
